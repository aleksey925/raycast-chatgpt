...
export const DEFAULT_AI_COMMANDS: Record<string, AiCommand> = {
  [FIX_SPELLING_AND_GRAMMAR_AI_COMMAND_ID]: {
...
export function useAiCommand(): AiCommandHook {
  ...
  const [data, setData] = useState<Record<string, AiCommand>>({});
  ...
  useEffect(() => {
    (async () => {
      ...
      if (Object.keys(storedCommands).length === 0) {
        ...
        Object.values(DEFAULT_AI_COMMANDS).forEach((cmd) => {
          addModel(mapCommandToModel(cmd));
        });
      } else {
        const allCommands: Record<string, AiCommand> = { ...storedCommands };
        Object.keys(DEFAULT_AI_COMMANDS).forEach((defaultKey) => {
          if (!storedCommands[defaultKey]) {
            allCommands[defaultKey] = DEFAULT_AI_COMMANDS[defaultKey];
          }
        });
        setData(allCommands);
  ...
  const isDefault = useCallback((id: string): boolean => {
    return id in DEFAULT_AI_COMMANDS;
  }, []);
